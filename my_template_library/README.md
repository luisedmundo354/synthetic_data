...
```
cicero parse --template ./second-amendment-assignment/ --sample ./second-amendment-assignment/text/sample.md
```

If the sample text parses successfully you will see the JSON representation of the contract data.

### Execute
Use the `cicero trigger` command to instantiate the clause using the sample text and invoke it with the default request.

```
cicero trigger --template ./second-amendment-assignment/ --sample ./second-amendment-assignment/text/sample.md --request ./second-amendment-assignment/request.json
```
